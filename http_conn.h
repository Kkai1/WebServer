...
{
public:
    static int m_epollfd;   //所有的socket上的事件都能被注册到同一个epoll上
    ...
    */
    ...
    void process();  //处理客户端请求  用于解析http请求
...
private:
    ...
    int m_read_index; //标识都缓冲区中以及读入的客户端数据的最后一个字节的下一个位置
    int m_checked_index;  //当前正在分析的字符在读读缓冲区的位置
